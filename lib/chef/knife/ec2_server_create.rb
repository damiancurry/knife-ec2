  ...
  class Knife
    class Ec2ServerCreate < Knife
      ...
      option :rdp_port,
        ...
        :default => "3389",
        :proc => Proc.new { |key| Chef::Config[:knife][:rdp_port] = key }
      ...
      def tcp_test_rdp(hostname, rdp_port)
        tcp_socket = TCPSocket.new(hostname, rdp_port)
        readable = IO.select([tcp_socket], nil, nil, 5)
        if readable
          yield
          true
      ...
      rescue SocketError, Errno::ECONNRESET, Errno::ECONNREFUSED, Errno::EHOSTUNREACH, Errno::ENETUNREACH, IOError
        sleep 2
        false
      rescue Errno::EPERM, Errno::ETIMEDOUT
        false
      ...
      def run
        ...
        # TODO:
        #   Find out what option was provided (ssh or rdp) and provide
        #   the correct output
      ...
      def bootstrap_for_windows_node(server,ssh_host)
      ...
      def wait_for_rdp(hostname)
        wait_for_direct_rdp(hostname, config[:rdp_port])
      end
